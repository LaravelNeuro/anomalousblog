...
Class Formatter extends Transition
{
    /**
    ...
    */
    ...
    public function __construct(int $projectId, TuringStrip $head, Collection $models)
    {
         ...
         */
        $this->setAgentById(Config::TRANSITION_FORMATTER_AGENT);     
    }
    ...
    {
        $this->promptData = json_decode($data);
        return $this->promptData[0]->article;
    }
    ...
    {
        $position = count($this->promptData);
        ...
        switch($position)
        {
            case 3:
                ...
                $blogEntry->articleEN = $data;
                $blogEntry->save();
                $this->head->setNext(TuringMove::REPEAT);
                array_shift($this->promptData);
                $data = json_encode($this->promptData);
                break;
            case 2:
                $blogEntry = BlogArticle::where('project_id', $this->project->id)->latest()->first();

                $blogEntry->articleDE = $data;
                $blogEntry->save();
                $this->head->setNext(TuringMove::NEXT);
                $data = $this->promptData[1]->image;
                break;
        }
