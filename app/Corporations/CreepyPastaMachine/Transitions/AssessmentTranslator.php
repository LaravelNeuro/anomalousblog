...
Class AssessmentTranslator extends Transition
{
    /**
    ...
    */
    ...
    public function __construct(int $projectId, TuringStrip $head, Collection $models)
    {
         ...
         */
        $this->setAgentById(Config::TRANSITION_FULLTEXT_AGENT);
    }
    ...
    {
        $data = ScpWarning::where('blog_id', BlogArticle::where('project_id', $this->project->id)
                                ->latest()
                                ->first()   
                                ->id)
                           ->where('lang', 'en')
                           ->latest()->first()->assessment;
        return $data;
    }
    ...
    {
        $ScpWarning = ScpWarning::where('blog_id', BlogArticle::where('project_id', $this->project->id)
                                ->latest()
                                ->first()   
                                ->id)
                           ->where('lang', 'en')
        ...
        $corporation = NetworkCorporation::with(['units.dataSetTemplates.dataSets'])->where('id',               
                                                $this->project->corporation_id)
                                                ->first();                                
        $classifications = json_decode($corporation
                                        ->units->where('name', 'SCPfoundation')
                                                ->first()
                                        ->dataSetTemplates->where('name', 'Classification')        
                                                ->first()
                                        ->dataSets->where('project_id', $this->project->id)
                                                ->first()
                                                ->data);
                                    
        $ScpWarning->containment = $classifications->containmentClass;
        $ScpWarning->clearance = $classifications->clearanceLevel;
        $ScpWarning->threat = $classifications->threatLevel;
        $ScpWarning->risk = $classifications->riskClass;
        $ScpWarning->disruption = $classifications->disruptionClass;

        $ScpWarning->save();

        $ScpWarning = $ScpWarning->replicate()
                                 ->fill([
                                    'lang' => 'de',
                                    'assessment' => $data
                                    ]);
        ...
        $ScpWarning->threat = $threatLevel[$classifications->threatLevel];
        $ScpWarning->risk = $riskClass[$classifications->riskClass];
        $ScpWarning->disruption = $disruptionClass[$classifications->disruptionClass];
